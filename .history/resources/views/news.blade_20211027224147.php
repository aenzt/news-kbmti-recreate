      ...
      <div class="flex flex-wrap -m-4">
          @foreach ($news as $berita)
            <div class="xl:w-1/3 md:w-1/2 p-4 filter drop-shadow-2xl">
                ...
                <h3 class="tracking-widest text-indigo-500 text-xs font-medium title-font">{{ $berita->bureau_id->bureau }}</h3>
                <h2 class="text-lg text-gray-900 font-medium title-font mb-4">{{ $berita->title }}</h2>
                <p class="leading-relaxed text-base">{{ $berita->body }}</p>
                </div>
            </div>
          @endforeach
      </div>
